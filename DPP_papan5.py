...
def dphi0(X,j,i):
    #print("sssssssssssss")
    vec=  2/np.pi *2*(i+1) * math.sin((i+1)*X[j])*math.cos((i+1)*X[j]) * 2/np.pi  * math.sin((i+1)*X[1-j])*math.sin((i+1)*X[1-j])
    #print("vec=",vec)
    return vec

def dphi(X,Y,j,i):
    #print("sssssssssssss")
    vec=  2/np.pi *(i+1) * math.cos((i+1)*X[j])*math.sin((i+1)*X[1-j]) * 2/np.pi  * math.sin((i+1)*Y[j])*math.sin((i+1)*Y[1-j])
    #print("vec=",vec)
    return vec
...
for j in range(2):
    ...
    for i in range(m):
        ...
        for s in range(len(X6)):
            ...
            phi=remove1(a,s)
            sum=B[s,s]*dphi0(X6[s],j,i)
            for t in phi:
                sum+=2*B[s,t]*dphi(X6[s],X6[t],j,i)
            #print(sum)
            Ci+=sum*F(X6[s],phi)
        C.append(Ci)
    A.append(C)
...
def dFnum(X,j,num):
    ...
    if num < 10:
        ...
        dsum=-X[j]**(num-11)/((X[j]**(num-10)+1)**2)
    return dsum
    ...
    for j in range(2):
        ...
        for i in range(m):
            ...
            for s in range(len(X6)):
                ...
                a=[l for l in range(len(X6))]
                phi=remove1(a,s)
                sum=BB[s][s]*dphi0(X6[s],j,i)
                for t in phi:
                    sum+=2*BB[s][t]*dphi(X6[s],X6[t],j,i)
                #print(sum)
                Ci+=sum*Fnum(X6[s],num+1)
            C.append(Ci)
        A.append(C)
