...
#mの選択をする。
#まずmを動かしたときの、lamを計算し、その時の密度を計算する。
#それをプロットする。
#m=40
#データ
...
def selc_true_lam(klist,c,m):
    lamli=[]
    for ki in klist:
        lamlist=[]
        for i in range(len(c)):
            lamlist.append(c[i]*ki.real/(c[i]*ki.real+1))
        if 0.99999 > lamlist[0] > 0.00001:
...
def make_data(m):
    ...
    filename='data_'+str(m)+'.csv'
    with open(filename,'w') as f:
        writer = csv.writer(f)
        for i in range(m):
            writer.writerow(philist[i])

    subprocess.call(["Rscript" ,"lam2.R" , filename])
...
if __name__ == '__main__':
    dens_m_list=[]
    mlist=range(len(X6),100)
    bar = tqdm(total = 70*35)
    for m in mlist:
        time.sleep(0.1)
        c=make_data(m)
        ...
        for sum_c in np.arange(0, 30, 1):
            ...
            true_lam=selc_true_lam(klist_real,c,m)
            print(true_lam)
            if not true_lam==[]:
        ...
        print("sum_c=",l1)
        dens_m_list.append([m,max_lam,max_dens,l1])
        if max_dens == -10000:
            break
    print(dens_m_list)
